...
Duplica (product name since 2026-08-10; was "Gráfica" — the project dir and `package.json` `name` deliberately still say "grafica", but the Postgres DB name and Docker container/image names were renamed to "duplica" on 2026-08-11): SEMED's copy/print request control system (solicitações de impressão/cópia from Escolas or Sede SEMED — production, delivery, status history, unit ranking, monthly consolidation). React 19 + Vite 6 + TypeScript frontend, **real backend** in `backend/` (Python/FastAPI + Postgres) — this used to be a `localStorage`-only MVP but that changed 2026-08-10 at explicit request. UI language is pt-BR only (`Intl.NumberFormat("pt-BR")`, `Intl.DateTimeFormat("pt-BR")`) — no i18n. No third-party UI component library on the content screens (dashboard, requests, units, users, reports); those style via a single global stylesheet (`src/styles.css`) with light/dark theme via a `data-theme` attribute on the root.
...
- Alembic migrations in `backend/alembic/versions/` — run `alembic revision --autogenerate` after changing a model, then review the generated migration before `alembic upgrade head`.
