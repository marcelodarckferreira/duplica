...
import { ArrowLeft, Camera, Eye, EyeOff, Pencil, Plus, Search, ShieldCheck, Trash2, UserCheck, UserX } from "lucide-react";
  ...
  return (
    <form className="grid gap-4 rounded-lg border border-border bg-surface p-[18px]" onSubmit={handleSubmit(onSubmit)}>
      ...
      <div className="mb-2.5">
        <p className="m-0 mb-2.5 text-[11px] font-bold uppercase tracking-wide text-muted">Dados da conta</p>
      ...
      <div>
        <p className="m-0 mb-2.5 text-[11px] font-bold uppercase tracking-wide text-muted">Senha e acesso</p>
        <div className="grid grid-cols-1 gap-1.5 sm:grid-cols-2 lg:grid-cols-4">
          ...
          <div className="grid min-w-0 gap-1.5 sm:col-span-2">
            ...
            <FieldError
              message={
                (typeof errors.confirmPassword?.types?.custom === "string" ? errors.confirmPassword.types.custom : undefined) ??
...
export function UsersView(props: {
  ...
  const filteredUsers = useMemo(() => {
    ...
    return users.filter((account) => {
      const matchesName = !normalizedQuery || account.name.toLowerCase().includes(normalizedQuery);
      const matchesRole = roleFilter === "Todos" || account.role === roleFilter;
      return matchesName && matchesRole;
    });
  }, [users, nameQuery, roleFilter]);
  ...
  return (
    <>
      <Card>
        <CardHeader>
          ...
          <div className="flex items-center gap-3.5">
            <CardDescription>
              {filteredUsers.length === users.length ? `${users.length} conta(s)` : `${filteredUsers.length} de ${users.length} conta(s)`}
            </CardDescription>
        ...
        <div className="mb-3.5 grid grid-cols-1 gap-2.5 sm:grid-cols-2 lg:grid-cols-[minmax(220px,1fr)_200px]">
          <label className="grid gap-1.5 text-xs font-bold uppercase text-label">
            <span className="invisible select-none" aria-hidden="true">Buscar</span>
            <span className="relative block">
          ...
          <label className="grid gap-1.5 text-xs font-bold uppercase text-label">
            Perfil
            <select value={roleFilter} onChange={(event) => setRoleFilter(event.target.value as UserRole | "Todos")} className={filterSelectClasses}>
              <option>Todos</option>
              {roles.map((role) => (
      ...
      <ConfirmModal
        ...
        description={
          pendingDelete
            ? `Tem certeza que deseja excluir a conta de ${pendingDelete.name}? Essa ação não pode ser desfeita.`
            : ""
        }
